...
contract BOBA3Core is ERC721Enumerable, ReentrancyGuard, Ownable, Pausable {
    ...
    function genPNG(BBT memory bbt) internal view returns (string memory) {
        ...
        string memory output = string(
            abi.encodePacked(z[0], z[1], back_layer, z[2])
        );
        output = string(
            abi.encodePacked(
                output,
        ...
        returns (string memory)
    {
        ...
        string memory json = string(
            abi.encodePacked('{"name": "BOBA3 NFT #', tokenId.toString(), '",')
        );

        json = string(
            abi.encodePacked(json, '"description": "Enjoy your BOBA3!",')
        );

        json = string(
            abi.encodePacked(
                json,
                '"attributes": [{"trait_type": "Bubbles", "value": "',
                uint256(bbt.is_bubbles).toString(),
                '"},',
                '{"trait_type": "Cream", "value": "',
                uint256(bbt.is_cream).toString(),
                '"},',
                '{"trait_type": "Tea_or_Latte?", "value": "',
                uint256(bbt.base).toString(),
                '"}'
            )
        );
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        Base64.encode(bytes(genPNG(bbt))),
                        '"}'
                    )
                )
            )
